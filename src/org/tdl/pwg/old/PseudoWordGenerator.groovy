...
def parseCommandLine(String[] args) {
    def result = new org.tdl.pwg.CommandLineArgs()
...
def ensureStartTokenMap(org.tdl.pwg.Model model, int effLen, String type) {
...
def ensureInnerTokenMap(org.tdl.pwg.Model model, int effLen, String prevType) {
...
def ensureLastTokenMap(org.tdl.pwg.Model model, int effLen, String prevType) {
...
def ensureTransitionMap(org.tdl.pwg.Model model, String key) {
...
def ensureSegmentList(org.tdl.pwg.Model model, String state) {
...
def analyzeText(String filePath, int ngramMode) {
    ...
    def model = new org.tdl.pwg.Model()
...
def validateModel(org.tdl.pwg.Model model, boolean requireSentenceStructure) {
    def errors = []
    ...
    if (errors) {
        throw new org.tdl.pwg.ModelValidationException("org.tdl.pwg.org.tdl.pwg.Model validation failed:\n  - ${errors.join('\n  - ')}")
    }
}

// --- Generation ---

def generatePseudoWord(org.tdl.pwg.Model model) {
    def fusedKey = selectFromWeightedMap(model.lengthStartStats)
    if (!fusedKey) {
        throw new org.tdl.pwg.GenerationException("Failed to select word length/type from empty lengthStartStats")
    }
    ...
    if (!startContent) {
        throw new org.tdl.pwg.GenerationException("Failed to select start token for effLen=${effLen}, type=${startType}")
    }
    tokens << startContent
    ...
    for (int i = 1; i < len; i++) {
        ...
        if (i == len - 1) {
            ...
            if (!tokenContent) {
                ...
                if (!lastMap || lastMap.isEmpty()) {
                    throw new org.tdl.pwg.GenerationException("No last token data for effLen=${effLen}, prevType=${prevType}")
                }
            ...
            if (!tokenContent) {
                ...
                if (!innerMap || innerMap.isEmpty()) {
                    throw new org.tdl.pwg.GenerationException("No inner token data for effLen=${effLen}, prevType=${prevType}")
                }
        ...
        if (!tokenContent) {
            throw new org.tdl.pwg.GenerationException("Failed to generate token at position ${i} for word length ${len}")
        }
...
def tryNgramSelection(org.tdl.pwg.Model model, int effLen, List<String> tokenContents, boolean isLast, int position) {
...
def generateSentences(org.tdl.pwg.Model model, int targetSentences) {
    ...
    while (sentencesGenerated < targetSentences) {
        ...
        if (!lenList || lenList.isEmpty()) {
            throw new org.tdl.pwg.GenerationException("No segment length data for state '${currentState}'")
        }
        ...
        if (!nextPunctMap || nextPunctMap.isEmpty()) {
            throw new org.tdl.pwg.GenerationException("No transition data for state '${currentState}'")
        }
...
def generateWordList(org.tdl.pwg.Model model, int count) {
    count.times { println generatePseudoWord(model) }
}

// --- Statistics ---

def printStatistics(org.tdl.pwg.Model model, boolean sentenceMode) {
    println "\n📊 --- ANALYSIS REPORT ---"
...
} catch (org.tdl.pwg.ModelValidationException e) {
    println "❌ MODEL VALIDATION FAILED:"
    println e.message
    System.exit(2)
} catch (org.tdl.pwg.GenerationException e) {
